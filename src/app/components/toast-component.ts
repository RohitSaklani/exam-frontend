...
import { ToastModel } from '../models/AppModel';
...
@Component({
  ...
  imports: [],
  template: `<div
    class="sm:space-y-6 sm:px-[2px] max-w-4xl mx-auto grid justify-center mt-6 fixed top-[80px] sm:top-[100px] right-[2px] z-100"
  >
    @if (toastService.toastList().length > 0) {
      @for (toast of toastService.toastList(); track toast.id) {
        <div class="toast-item" [class.removing]="!toast.visible">
          <div
            [class.show]="toast.visible"
            class="toast bg-white p-4 m-0 mb-1 rounded-md shadow-xs border border-slate-300 min-w-xs max-w-sm relative dark:bg-neutral-800 dark:border-neutral-700"
            role="alert"
          >
            <div class="flex justify-center items-start gap-2.5">
              @if (toast.variant == 'success') {
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="size-[18px] fill-green-700 overflow-visible dark:fill-green-400"
                  viewBox="0 0 330 330"
                  aria-hidden="true"
                >
                  <path
              ...
              } @else if (toast.variant == 'alert') {
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="size-[18px] fill-yellow-700 overflow-visible dark:fill-yellow-400"
                ...
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="size-[18px] fill-red-700 overflow-visible dark:fill-red-400"
                  viewBox="0 0 512 512"
                  aria-hidden="true"
                >
                  <path
                ...
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="size-[18px] fill-blue-700 overflow-visible dark:fill-blue-400"
                  viewBox="0 0 330 330"
                  aria-hidden="true"
                >
                  <path
              ...
              <div>
                <!-- <p class="text-sm text-slate-900 font-medium dark:text-slate-50 leading-tight">
                {{ toast.id }}
              </p> -->
              ...
              <button
                ...
                class="ml-auto flex items-center opacity-70 hover:opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded"
                (click)="toastService.customRemove(toast.id)"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="size-2.5 cursor-pointer fill-slate-500 dark:fill-neutral-400"
                  aria-hidden="true"
